...
const generateEncryptedPrivateKey = async (password: string, privateKey: CryptoKey): Promise<ArrayBuffer> => {
...
const decryptPrivateKey = async (password: string, encryptedPrivateKey: string): Promise<CryptoKey> => {
  ...
  const keyData: JsonWebKey = { ...json, crv: "P-384", kty: "EC" };
  return await crypto.subtle.importKey("jwk", keyData, { name: "ECDSA", namedCurve: "P-384" }, false, ["sign"]);
};

const signString = async (password: string, encryptedPrivateKey: string, data: string): Promise<string> => {
...
const generateKey = async (hashKeyPassphrase = false): Promise<Key> => {
  const keyPair = await generateKeyPair();
  const exportedPublicKey = await crypto.subtle.exportKey("spki", keyPair.publicKey);

  const password = generatePassword();
  const encryptedPrivateKey = await generateEncryptedPrivateKey(password, keyPair.privateKey);
  const digestedPublicKey = await crypto.subtle.digest("SHA-256", exportedPublicKey);
  ...
  const keyObj: Key = {
    ...
    } as ServiceAccountKeyPair,
    password
  };

  if (hashKeyPassphrase) {
    const hashedPassphrase = await crypto.subtle.digest("SHA-512", new TextEncoder().encode(password));

    const encodedKeyId = new TextEncoder().encode(keyObj.key.keyId);
    const hashedCombination = await crypto.subtle.digest("SHA-512", concatArrayBuffers(encodedKeyId, hashedPassphrase));

    const hashArray = Array.from(new Uint8Array(hashedCombination));

    keyObj["key"]["hashedKeyPassphrase"] = hashArray.map(b => b.toString(16).padStart(2, "0")).join("");
  }
